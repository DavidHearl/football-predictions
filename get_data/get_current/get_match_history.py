...
class MatchHistory:
	...
	def create_match_folders(self):
		...
		for league in club_urls:
			...
			folder_count[league] = sum(os.path.isdir(os.path.join(location, entry.name)) for entry in os.scandir(location))
			match_count += folder_count[league] * ((folder_count[league] - 1)*2)
		...
		for league in club_urls:
			...
			for folder in os.listdir(location):
				...
				for match in data:
					...
					time_remaining = round(((match_count - current_count) * 4) / 60, 2)
					print(f"Match {current_count} of {match_count} - {percentage}% complete - {time_remaining} minutes remaining")

					if suspended != 'Match Suspended':
						# Add a delay to stop the server from blocking the request
						random_number = round(random.uniform(3, 5), 1)
						print(f"Current Time: {time.strftime('%H:%M:%S', time.localtime())}, Waiting {random_number} seconds...")
						print(f"Creating Match Report for {folder} vs {opponent} - {home_away} in {league}, {self.season} season\nUrl: {match_url}")
						...
						try:
							...
							# Ensure lineup_div_home is not None before calling find all
							if lineup_div_home is not None:
								td_tags_home = lineup_div_home.find_all('td')
							else:
								td_tags_home = []

							# Ensure lineup_div_away is not None before calling find all
